...
func initGpu(config *Config) (*GPUContext, error) {
	// Build concatenated pattern buffer + lengths
	...
	privateKeyBuffer, err := runner.CreateEmptyBuffer(cl.WRITE_ONLY, batchSize*64)
	if err != nil {
	...
	patternBuffer, err := runner.CreateEmptyBuffer(cl.READ_ONLY, len(allPatterns))
	if err != nil {
		return nil, fmt.Errorf("failed to create pattern buffer: %v", err)
	}

	// One byte per pattern length - simple!
	patternLenBuffer, err := runner.CreateEmptyBuffer(cl.READ_ONLY, len(patternLengths))
	if err != nil {
...
func (g *GPUContext) findVanityKeysGPU(config *Config) (*resultFound, error) {
	...
	args := []cl.KernelParam{
		...
		cl.BufferParam(g.patternLenBuffer),
		cl.Param(&batchSizeParam),
		cl.Param(&numPatternsParam),
		cl.Param(&locationInt),
		cl.Param(&ignoreCaseInt),
	}

	err = g.runner.RunKernel("sleipnir_ed25519_keygen", 1, nil,
		[]uint64{g.globalWorkSize}, []uint64{g.workGroupSize}, args, true)
	if err != nil {
	...
	foundPrivateKeys := make([]byte, 64)
	err = cl.ReadBuffer(g.runner, 0, g.privBuffer, foundPrivateKeys)
	if err != nil {
		return nil, fmt.Errorf("failed to read private keys: %v", err)
	}

	// foundPrivateKeys is now [seed(32) || pubkey(32)] = valid Go ed25519.PrivateKey
	privKey := ed25519.PrivateKey(foundPrivateKeys)

	data, err := privateKeyToOpenSSH(privKey, "")
	if err != nil {
		return nil, err
	}
	...
	pubString := "ssh-ed25519 " + publicKeyToSSHFormat(ed25519.PublicKey(foundPublicKeys), buf)
